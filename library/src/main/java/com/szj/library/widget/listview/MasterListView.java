package com.szj.library.widget.listview;
...
public class MasterListView extends ListView implements OnScrollListener {
	...
	// -- header view
	private MTFListViewHeader mHeaderView;
	...
	// -- footer view
	private MTFListViewFooter mFooterView;
	...
	private void initWithContext(Context context) {
		...
		// init header view
		mHeaderView = new MTFListViewHeader(context);
		...
		// init footer view
		mFooterView = new MTFListViewFooter(context);
	...
	public void setPullLoadEnable(boolean enable) {
		...
		if (!mEnablePullLoad) {
			...
			mPullLoading = false;
			mFooterView.show();
			mFooterView.setState(MTFListViewFooter.STATE_NORMAL);
	...
	public void startRefresh(){
		...
		mHeaderView.startRotateProgress();
		mHeaderView.setState(MTFListViewHeader.STATE_REFRESHING);
	}
	...
	private void updateHeaderHeight(float delta) {
		...
		if (mEnablePullRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
			if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
				mHeaderView.setState(MTFListViewHeader.STATE_READY);
			} else {
				mHeaderView.setState(MTFListViewHeader.STATE_NORMAL);
			}
		}
	...
	private void updateFooterHeight(float delta) {
		...
		if (mEnablePullLoad && !mPullLoading) {
			if (height > PULL_LOAD_MORE_DELTA) { // height enough to invoke load
													// more.
				mFooterView.setState(MTFListViewFooter.STATE_READY);
			} else {
				mFooterView.setState(MTFListViewFooter.STATE_NORMAL);
			}
		}
...
//		setSelection(mTotalItemCount - 1); // scroll to bottom
	...
	public void startLoadMore() {
		mPullLoading = true;
		mFooterView.setState(MTFListViewFooter.STATE_LOADING);
...
//			System.out.println("计算刷新时间");
	...
	public interface OnMasterScrollListener extends OnScrollListener {
		void onMasterScrolling(View view);
	}
